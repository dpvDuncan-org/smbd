...
SHARES_CONFIG_FILE="/config/shares.conf"
...
EOH
    ...
    u)
      ...
      ;;
    s)
      IFS=: read sharename sharepath readwrite users otheropts <<< "${OPTARG}"
      if [[ ! -f ${SHARES_CONFIG_FILE} ]] || [[ -f ${SHARES_CONFIG_FILE} && ! $(grep '\['${sharename}'\]' ${SHARES_CONFIG_FILE}) ]]
      then
        echo -n "Add share '${sharename}' "
        echo "[${sharename}]" >> "${SHARES_CONFIG_FILE}"
        echo -n "path '${sharepath}' "
        echo "  path = \"${sharepath}\"" >> "${SHARES_CONFIG_FILE}"
        echo -n "read"
        if [[ "rw" = "${readwrite}" ]]
        then
          echo -n "+write "
          echo "  read only = no" >> "${SHARES_CONFIG_FILE}"
          echo "  writable = yes" >> "${SHARES_CONFIG_FILE}"
        else
          echo -n "-only "
          echo "  read only = yes" >> "${SHARES_CONFIG_FILE}"
          echo "  writable = no" >> "${SHARES_CONFIG_FILE}"
        fi
        if [[ -z "${users}" ]]
        then
          echo -n "for guests: "
          echo "  browseable = yes" >> "${SHARES_CONFIG_FILE}"
          echo "  guest ok = yes" >> "${SHARES_CONFIG_FILE}"
          echo "  public = yes" >> "${SHARES_CONFIG_FILE}"
        else
          echo -n "for users: "
          users=$(echo "${users}" |tr "," " ")
          echo -n "${users} "
          echo "  valid users = ${users}" >> "${SHARES_CONFIG_FILE}"
          echo "  write list = ${users}" >> "${SHARES_CONFIG_FILE}"
        fi
        if [[ "${otheropts}" ]]
        then
          IFS=',' read -r -a otheropts_list <<< "${otheropts}"
          for otheropt in "${otheropts_list[@]}"
          do
            echo "  ${otheropt}" >> "${SHARES_CONFIG_FILE}"
          done
        fi
        echo "DONE"
      else
        echo "Share '${sharename}' already exists, using config parameters"
      fi
      ;;
    \?)
...
exec ionice -c 3 smbd --configfile="${CONFIG_FILE}" -l /config -F --debug-stdout --no-process-group